...
// #endif
// };
volatile const struct gyro_reg_s reg = {
    0x75, // who_am_i
    0x19, // rate_div
    0x1A, // lpf
    0x0C, // prod_id
...
//     .max_accel_var  = 0.14f
// };
volatile const struct test_s test = {
    32768 / 250, // gyro_sens
    32768 / 16,  //	accel_sens
    0,           //	reg_rate_div
    1,           //	reg_lpf
...
//     .test = &test
// };
volatile static struct gyro_state_s st = {
    &reg,
    &hw,
    {0},
    &test};
...
uint8_t mpu_dmp_get_data(float *pitch, float *roll, float *yaw)
{
    ...
    if (sensors & INV_WXYZ_QUAT)
    {
        ...
        q3 = quat[3] / q30;
        // 计算得到俯仰角/横滚角/航向角
        *pitch = asinf(-2.0f * q1 * q3 + 2.0f * q0 * q2) * 57.3f;                              // pitch
        *roll = atan2f(2.0f * q2 * q3 + 2.0f * q0 * q1, -2.0f * q1 * q1 - 2.0f * q2 * q2 + 1.0f) * 57.3f; // roll
        *yaw = atan2f(2.0f * (q1 * q2 + q0 * q3), q0 * q0 + q1 * q1 - q2 * q2 - q3 * q3) * 57.3f; // yaw
    }
    else
        return 2;
    return 0;
